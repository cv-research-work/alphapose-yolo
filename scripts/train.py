...
def main():
    ...
    for i in range(cfg.TRAIN.BEGIN_EPOCH, cfg.TRAIN.END_EPOCH):
        ...
        if (i + 1) % opt.snapshot == 0:
            ...
            # Prediction Test
            with torch.no_grad():
                val_loss, val_acc = validate_gt(m.module, opt, cfg, heatmap_to_coord,criterion)
    ...
    ## Plot
    plt.plot(range(cfg.TRAIN.BEGIN_EPOCH, cfg.TRAIN.END_EPOCH),train_acc)
    plt.plot(val_epochs,val_accuracy)
    plt.legend(["train acc","val acc"])
    plt.savefig("learning_curve_acc.png")

    fig = plt.figure()    
    plt.plot(range(cfg.TRAIN.BEGIN_EPOCH, cfg.TRAIN.END_EPOCH),train_losses)
